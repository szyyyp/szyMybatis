...
import com.bupt.result.page.LtrFilter;
...
public class PageUtil {
    public static <T>Map<String, Object> page(LtrPageable select) throws IllegalAccessException {
        ...
        if (select.getStringOrder() != null) {
            select.setOrder(LtrPageable.fromString(select.getStringOrder()));
        }
        where.put("order", select.getOrder().toString());
        //新建 list 以供后续存储查询条件并处理为 beanSearcher 可识别的形式
        List<LtrFilter> filters = new ArrayList<>();
        //初始化 new list
        Collections.addAll(filters, new LtrFilter[select.getFilters().size()]);
        //copy 前端 DTO 中查询 list 以供处理
        Collections.copy(filters, select.getFilters());
        //循环将查询及查询条件放入 map 交给 beanSearcher 处理
        for (LtrFilter f : filters) {
            //判断是否携带运算符 必须携带运算符才能进行筛选
            if (f.getStringOperator() != null) {
                ...
                if (f.getStringOperator().equals("bt") || f.getStringOperator().equals("nb") || f.getStringOperator().equals("il")) {
                    ...
                    if (f.getValue() instanceof List) {
                        ...
                        f.setOperator(LtrFilter.fromString(f.getStringOperator()));
